...
/* Il predicato acquisisci_polinomio acquisisce un polinomio di coefficienti Double da tastiera,
   ...
   - il primo argomento è l'etichetta (nome) del polinomio da acquisire, usata nel messaggio
     mostrato all'utente;
...
/* Il predicato elabora_input converte l'input testuale in coefficienti numerici tramite converti_input,
   ...
   - il primo argomento è l'etichetta del polinomio, usata per richiedere nuovamente l'input
     in caso di errore;
   - il secondo argomento è la lista delle stringhe ottenute dalla suddivisione della riga letta;
...
% Il predicato ausiliario converti_input/3 converte ricorsivamente la lista di stringhe in coefficienti,
% accumulando il risultato in ordine inverso:
% - il primo argomento è la lista delle stringhe residue da convertire;
% - il secondo argomento è l'accumulatore dei coefficienti convertiti finora (in ordine inverso);
% - il terzo argomento è la lista dei coefficienti risultante.
...
/* Il predicato rimuovi_zeri_in_testa elimina gli zeri di testa (grado massimo) di un polinomio:
   ...
   - il secondo argomento è la lista dei coefficienti risultante, priva degli zeri di testa. */
...
% - il primo argomento è la lista dei coefficienti in ordine invertito (dal grado massimo al minimo);
% - il secondo argomento è la lista risultante, priva dei coefficienti di testa sotto tolleranza.
...
/* Il predicato grado_polinomio calcola il grado di un polinomio come la lunghezza del polinomio
...
/* Il predicato mostra stampa la rappresentazione algebrica di un polinomio, dal grado massimo al minimo.
   ...
   - il suo unico argomento è la lista dei coefficienti del polinomio da stampare, in ordine crescente
     di grado. */
mostra(Polinomio) :-
    rimuovi_zeri_in_testa(Polinomio, Poli_norm),
...
% - il primo argomento è la lista dei coefficienti in ordine crescente di grado;
% - il secondo argomento è la lista delle coppie (grado, coefficiente) risultante, ordinata
...
% - il primo argomento è la lista dei coefficienti residui da associare al grado;
% - il secondo argomento è il grado corrente, incrementato a ogni passo della ricorsione;
...
% - il secondo argomento è un flag (1 oppure 0) che indica se il termine da stampare è il
...
% - il secondo argomento è il flag (1 oppure 0) che indica se il termine è il primo termine
...
/* Il predicato somma calcola la somma di due polinomi in tempo lineare O(n):
   ...
   - il terzo argomento è la lista dei coefficienti del polinomio somma risultante. */
...
% - il primo argomento è la lista dei coefficienti del primo polinomio;
% - il secondo argomento è la lista dei coefficienti del secondo polinomio;
% - il terzo argomento è la lista dei coefficienti risultante dalla somma termine a termine.
somma_liste([], P2, P2) :- !.
somma_liste(P1, [], P1) :- !.
...
/* Il predicato differenza calcola la differenza tra due polinomi in tempo lineare O(n):
   ...
   - il terzo argomento è la lista dei coefficienti del polinomio differenza risultante. */
...
% - il primo argomento è la lista dei coefficienti del primo polinomio (minuendo);
% - il secondo argomento è la lista dei coefficienti del secondo polinomio (sottraendo);
% - il terzo argomento è la lista dei coefficienti risultante dalla differenza termine a termine.
...
/* Il predicato prodotto calcola il prodotto di due polinomi:
   - il primo argomento è la lista dei coefficienti del primo polinomio;
   - il secondo argomento è la lista dei coefficienti del secondo polinomio;
   - il terzo argomento è la lista dei coefficienti del polinomio prodotto risultante. */
...
% - il primo argomento è la lista dei coefficienti del primo polinomio;
% - il secondo argomento è la lista dei coefficienti del secondo polinomio;
% - il terzo argomento è la lista dei coefficienti risultante dal prodotto, non normalizzata.
prodotto_liste([], _, []) :- !.
prodotto_liste(_, [], []) :- !.
...
/* Il predicato divisione calcola quoziente e resto della divisione euclidea tra due polinomi:
   ...
   - il terzo argomento è la lista dei coefficienti del polinomio quoziente risultante;
   - il quarto argomento è la lista dei coefficienti del polinomio resto risultante. */
...
% - il quarto argomento è la lista dei coefficienti del quoziente finale risultante;
% - il quinto argomento è la lista dei coefficienti del resto finale risultante.
...
% - il terzo argomento è la lista dei coefficienti risultante, rappresentante il monomio.
...
/* Il predicato mcd calcola il massimo comun divisore di due polinomi tramite l'algoritmo
   ...
   - il primo argomento è la lista dei coefficienti del primo polinomio;
   - il secondo argomento è la lista dei coefficienti del secondo polinomio;
   - il terzo argomento è la lista dei coefficienti del massimo comun divisore risultante. */
